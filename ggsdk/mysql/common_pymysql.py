...
from redis._compat import Full
from redis import BlockingConnectionPool

from pymysql.constants import CR
from . import converters
from ggsdk import utility
...
MAX_UNSIGNED_MEDIUMINT = 16777215
MIN_MEDIUMINT = -8388608
MAX_MEDIUMINT = 8388607

MAX_UNSIGNED_INT = 4294967295
MIN_INT = -2147483648
MAX_INT = 2147483647

MAX_UNSIGNED_BIGINT = 4294967295
...
class MysqlConnection(object):
    ...
    def connect(self, host, port, dbuser, dbpass, dbname, dic_cur, enc, logger, check_dbname=True):
        """
        ...
        if not self.valid:
            t1 = time.time()
            logger.debug('pymysql.connect begin host=%s, port=%s, db=%s' % (host, port, dbname))
            ...
            logger.debug('pymysql.connect done host=%s, port=%s, db=%s' % (host, port, dbname))
    ...
    def query(self, sqlstr, sqlargs=(), logger=None, **kwargs):
        t1 = time.time()
        ...
        if len(cmd) >= max_allowed_packet:
            logger = logger if logger is not None else utility.get_default_logger()
            # TODO: change to raise Exception
            logger.exception(
                'sql=%s is toooo long(max=%s) %s' % (len(cmd), max_allowed_packet, cmd[:300]))
        self.cursor.execute(cmd)
        rs = self.cursor.fetchall()
        rows = []
        for row in rs:
            conv_row = {utility.unicode_to_str(i): j for i, j in six.iteritems(row)}
...
class ConnectionPool(BlockingConnectionPool):
    def __init__(self, cls_name, pool_size,
        ...
        self.dic_cur = dic_cur
        self.enc = enc
        self.logger = logger if logger is not None else utility.get_default_logger()
    ...
    def _safe_run(self, func, action='', defval=None, verbose=False, **kwargs):
        ...
        try:
            connection.connect(host=self.host, port=self.port, dbuser=self.dbuser,
                               ...
                               enc=self.enc, logger=self.logger, check_dbname=self.check_dbname)
            t3 = time.time()
            if t3 - t2 > 0.2:
            ...
            # 缺表、缺行、执行错误
            self.logger.error('taou_pymysql execute_command1(%s) err=%s, sqlstr=%s, sqlargs=%s' % (
            ...
            self.release(connection)
            self.logger.debug('_safe_run on %s use %s spends %.4f ms' % (self.host, self._cls, (time.time() - t1) * 1000))
...
class MysqlClientPool(ConnectionPool):
    def __init__(self, pool_size=1, cls_name=None, host='localhost', port=3306, dbname='', readonly=True,
        ...
        if logger is None:
            logger = utility.get_default_logger()
    ...
    def filter_valid_fields(self, val_dict, table):
        ...
        for k, v in six.iteritems(val_dict):
            if k in fields:
                val_dict2[k] = utility.unicode_to_str(v)
        return val_dict2
